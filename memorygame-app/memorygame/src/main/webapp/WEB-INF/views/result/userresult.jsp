...
<%@ page import="hu.icell.common.dto.*" %>
...
<div id="body">
	<div id="nav">
		<ul>
			...
			<li><a class="active" href="<%= request.getContextPath() %>/game/result/<%= ((UserDTO)request.getSession().getAttribute("user")).getId() %>">My results</a></li>
	...
	<% 
		...
		if (list != null && !list.isEmpty()) {
			...
			for(ResultDTO r: list) {%>
				<tr><th><%= r.getId() %></th><th><%= dateFormat.format(r.getResultDate()) %></th><th><%= r.getSeconds() %></th><th><%= r.getUsername() %></th></tr>
			<%
			}
			out.println("</table>");
		}// else {
